...
require_cmd(){ command -v "$1" >/dev/null 2>&1 || { err "Benötigtes Kommando fehlt: $1"; exit 1; }; }
...
setup_config_interactive(){
  load_config
  echo
  echo "Konfiguration prüfen / anpassen"
  SERVICE_NAME="$(prompt_default 'Systemd Service Name' "$SERVICE_NAME")"
  CLI_BIN="$(prompt_default 'Pfad defcon-cli' "$CLI_BIN")"
  DAEMON_BIN="$(prompt_default 'Pfad defcond' "$DAEMON_BIN")"
  DATA_DIR="$(prompt_default 'Datadir' "$DATA_DIR")"
  CONF_FILE="$(prompt_default 'Pfad defcon.conf' "$CONF_FILE")"
  NODE_USER="$(prompt_default 'Node Linux User' "$NODE_USER")"
  INTERVAL="$(prompt_default 'Messintervall in Sekunden' "$INTERVAL")"
  EVENT_INTERVAL="$(prompt_default 'Event-Intervall in Sekunden' "$EVENT_INTERVAL")"
  JOURNAL_LINES="$(prompt_default 'Initiale Journal-Zeilen' "$JOURNAL_LINES")"
  RETENTION_DAYS="$(prompt_default 'Aufbewahrungstage Logs' "$RETENTION_DAYS")"
  PEER_SAMPLE_MAXKB="$(prompt_default 'Max KB pro Peer-Sample' "$PEER_SAMPLE_MAXKB")"
  cat > "$CFG_FILE" <<EOF2
...
EOF2
  echo "Konfiguration gespeichert in $CFG_FILE"
}
...
collect_cli_snapshot(){
  local out="$1"
  {
    ...
    if [ -x "$CLI_BIN" ]; then
      ...
      echo "CLI nicht gefunden: $CLI_BIN"
    fi
  } > "$out" 2>&1
}
...
flock -n 9 || { echo "monitor läuft bereits"; exit 1; }
...
while true; do
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,"%s"\n' \
    "$ts" "$load1" "$load5" "$load15" "$cpu_user" "$cpu_system" "$cpu_idle" "$mem_total" "$mem_avail" "$swap_total" "$swap_free" "$root_use" "$datadir_use" "$rx_bytes" "$tx_bytes" "$established" "$defcond_cpu" "$defcond_mem" "$defcond_rss" "$defcond_threads" "$defcond_fd_count" "${chain_blocks:-}" "${headers:-}" "${verificationprogress:-}" "${connections:-}" "${mn_synced:-}" "${mn_state:-}" >> "$TS_CSV"
...
stop_monitor(){
  ...
  log "Alle Hintergrundprozesse gestoppt"
}
...
score_line(){ score=$((score+$1)); }
{
  ...
  if [ -f "$TS_CSV" ]; then
    ...
    low_conn=$(awk -F',' 'NR>1 && ($25=="" || $25+0<3){c++} END{print c+0}' "$TS_CSV")
    lagging=$(awk -F',' 'NR>1 && $22!="" && $23!="" && ($23-$22)>3 {c++} END{print c+0}' "$TS_CSV")
    echo "- Max load1: $max_load"
    echo "- Max defcond CPU %: $max_cpu"
    echo "- Max defcond MEM %: $max_mem"
    echo "- Max offene File Descriptors: $max_fd"
    echo "- Min MemAvailable KB: $min_mem"
    echo "- Samples mit sehr wenigen Verbindungen (<3): $low_conn"
    echo "- Samples mit Header/Block-Lag > 3: $lagging"
    ...
    echo "- Keine Zeitreihe gefunden"
    score_line 5
  fi
  echo
  echo "## Risikoeinschätzung"
  if [ "$score" -le 2 ]; then
    echo "- Niedrig: keine klaren VPS-Hinweise auf Ressourcenengpässe sichtbar."
  elif [ "$score" -le 5 ]; then
    echo "- Mittel: einzelne Ressourcen- oder Connectivity-Auffälligkeiten vorhanden."
  else
    echo "- Hoch: VPS oder Node-Verhalten zeigen deutliche Hinweise auf Instabilität."
  fi
  echo
  echo "## Relevante Alerts"
  if [ -f "$ALERTS_CSV" ]; then
    tail -n 200 "$ALERTS_CSV" | sed 's/^/- /'
  else
    echo "- Keine Alerts-Datei gefunden"
  fi
  echo
  echo "## Journal Muster"
  if [ -f "${LOG_DIR}/journal-follow.log" ]; then
    ...
    echo "- Kein Journal-Follow vorhanden"
  fi
  echo
  echo "## Empfehlungen"
  echo "- Prüfe, ob PoSe/DKG-Ereignisse zeitlich mit wenig Peers, Header-Lag, hoher Load oder knappem RAM korrelieren."
  echo "- Wenn wiederholt Reindex + Clean State nötig wird, prüfe Peer-Qualität und falsche/forked Peers zuerst."
  echo "- Änderungen an Quorum/PoSe-Parametern erst nach ausreichender Messdauer und mit klarer Vorher/Nachher-Basis testen."
  echo "- Falls der Host oft in knappen RAM oder hohe I/O-Wartezeiten läuft, stärkere VPS-Klasse gegenprüfen."
} > "$REPORT_MD"
{
  echo "DFCN Kurzbericht"
  echo "Erstellt: $(date -Is)"
  echo
  [ -f "$TS_CSV" ] && echo "letzte 15 Zeilen timeseries.csv" && tail -n 15 "$TS_CSV"
  echo
  [ -f "$ALERTS_CSV" ] && echo "letzte 50 Alerts" && tail -n 50 "$ALERTS_CSV"
  echo
  [ -f "${LOG_DIR}/pose-events.log" ] && echo "letzte 80 PoSe-bezogene Einträge" && tail -n 80 "${LOG_DIR}/pose-events.log"
} > "$REPORT_TXT"
...
generate_report(){ write_analyze_script; mapfile -t generated < <("$ANALYZE_SCRIPT"); log "Reports erstellt:"; printf ' - %s\n' "${generated[@]}"; }
cleanup_all(){ read -r -p "Wirklich alle Inspector-Daten löschen? [yes/NO]: " ans || true; [ "$ans" = "yes" ] || { log "Abgebrochen"; return 0; }; stop_monitor || true; rm -rf "$BASE_DIR"; log "Alles gelöscht: $BASE_DIR"; }
show_status(){ load_config; echo; echo "=== STATUS ==="; echo "Base dir: $BASE_DIR"; echo "Service : $SERVICE_NAME"; echo "CLI     : $CLI_BIN"; echo "Datadir : $DATA_DIR"; for f in "$PID_FILE" "$TAIL_PID_FILE" "$EVENT_PID_FILE"; do [ -f "$f" ] && echo "$(basename "$f"): $(cat "$f")" || true; done; echo; }
selftest(){ load_config; echo "Grundkommandos:"; for c in awk sed grep ps ss systemctl journalctl nohup timeout date df free ip find flock ionice nice; do if have_cmd "$c"; then echo "OK  $c"; else echo "MISS $c"; fi; done; echo; echo "Service:"; systemctl is-active "$SERVICE_NAME" || true; echo; echo "Datadir:"; ls -ld "$DATA_DIR" 2>/dev/null || true; echo; echo "CLI Probe:"; timeout 15 "$CLI_BIN" -datadir="$DATA_DIR" -conf="$CONF_FILE" getblockchaininfo 2>/dev/null | head -n 20 || true; }
usage(){ cat <<EOF2
$APP_NAME v$VERSION

Menü:
  1) Untersuchung starten und loggen
  2) Alles stoppen und Auswertung erstellen
  3) Bereinigung: Script-Daten löschen
  4) Konfiguration anzeigen / anpassen
  5) Status anzeigen
  6) Schnelltest
...
menu(){ while true; do usage; echo; read -r -p "Auswahl: " choice || true; case "$choice" in 1) setup_config_interactive; start_monitor; break ;; 2) stop_monitor; generate_report; break ;; 3) cleanup_all; break ;; 4) setup_config_interactive ;; 5) show_status ;; 6) selftest ;; *) echo "Ungültige Auswahl" ;; esac; echo; done; }
case "${1:-}" in start) setup_config_interactive; start_monitor ;; stop-report) stop_monitor; generate_report ;; cleanup) cleanup_all ;; config) setup_config_interactive ;; status) show_status ;; selftest) selftest ;; "") menu ;; *) usage; exit 1 ;; esac
